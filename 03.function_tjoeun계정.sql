/* 
    ...
    >> 함수식을 기술할 수 있는 위치 : SELECT절, WEEK절, ORDER BY절
*/
-----------------------------단일행 함수-----------------------------
===================================================================
--                           문자처리 필수
...
/*
    ...
    LENGTH(컬럼|'문자열') : 해당 문자열의 byte수 반환
...
*/
 ...
 SELECT EMP_NAME, LENGTH(EMP_NAME), LENGTHB(EMP_NAME), 
        EMAIL, LENGTH(EMAIL), LENGTHB(EMP_NAME)
  FROM EMPLOYEE;
  
-------------------------------------------------------------------
/*
    * INSTA : 문자열로부터 특정문자와 시작위치(INDEX)를 찾아서 반환(반환형:NUMBER)
        - ORACLE에서 INDEX번호는 1부터 시작, 찾을 문자가 없으면 0반환
        
        [표현법]
        INDEX(컬럼|'문자열', '찾고자하는 문자', [찾을위치의 시작값, [순번]])
            - 찾을위치 시작값
             1 : 앞에서부터 찾기(기본값)
             -1 : 뒤에서부터 찾기
*/
 ...
 SELECT EMAIL, INSTR(EMAIL, '@')
  ...
  /*
    * LPAD / RPAD : 문자열을 조회할 때 통일감있게 조회하고자 할 때(O반환형 : CHARACTER)
     ...
     [표현법]
     LPAD/KPAD('문자열', 최종적으로 반환할 문자의 길이, [덧붙이고자하는 문자])
        - 문자열에 덧붙이고자하는 문자를 왼쪽 또는 오른쪽에 덧붙여서 최종 N길이만큼의 문자열 반환
 -------------------------------------------------------------------------------
*/
-- EMPLOYEE에서 사원명, 이메일(길이25, 오른쪽 정렬)
...
/*
    ...
    *TRIM : 문자열에서 앞/뒤 양쪽에 있는 특정문자를 제거한 나머지를 반환
    ...
    TRIM[LEADING|TRALING|BOTH]제거하고자하는 문자들 FROM '문자열')
    
    문자열의 왼쪽 또는 오른쪽으로 제거하고자하는 문자들을 찾아서 제거한 나머지 문자열 반환
*/
-- 제거하고자하는 문자를 넣지 않으면 공백 제거
SELECT LTRIM('     TJOEUN     ') || '학원' FROM DUAL;
SELECT RTRIM('     TJOEUN     ') || '학원' FROM DUAL;

SELECT LTRIM('JAVAJAVASCRIPT', 'JAVA') FROM DUAL;
SELECT LTRIM('BACACABCABCBCC', 'ABC') FROM DUAL;
SELECT LTRIM('41654891513', '0123456879') FROM DUAL;

SELECT RTRIM('BACACABCABCBCC', 'ABC') FROM DUAL;
SELECT RTRIM('32454621654562', '0123456789') FROM DUAL;

-- BOTH가 기본값 : 양쪽제거
SELECT TRIM('     TJOEUN     ') || '학원' FROM DUAL;
SELECT TRIM('A' FROM'AAADSFSFKJSFHAAA') FROM DUAL; --1글자만 가능
SELECT TRIM(BOTH 'A' FROM'AAADSFSFKJSFHAAA') FROM DUAL;

SELECT TRIM(LEADING 'A' FROM'AAADSFSFKJSFHAAA') FROM DUAL;
SELECT TRIM(TRAILING 'A' FROM'AAADSFSFKJSFHAAA') FROM DUAL;
-------------------------------------------------------------------
/*
...
SELECT EMP_NAME, HIRE_DATE, CONCAT(CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)),'개월차' "근무개월수"
 ...
 ---------------------------------------------------------------
 /*
    * ADD_MONTHS(DATE, NUMBER) : 특정날짜에 해당 숫자만큼 개월수로 더해 반환
 */
 
 SELECT ADD_MONTHS(SYSDATE, 1) FROM DUAL;
 
 -- EMPOLYEE에서 사원명, 입사일, 입사후 정직원 날짜(입사 후 6개월) 조회
 SELECT EMP_NAME, HIRE_DATE, ADD_MONTHS(HIRE_DATE, 6) "정직원된 날짜"
 FROM EMPLOYEE;
 
 -----------------------------------------------------------------------
 /*
    * NEXT_DAY(DATE, 요일[문자|숫자|] : X특정 날짜 이후에 가까운 해당 요일의 날짜를 반환해주는 함수
        -1 : 일요일
*/
...
SELECT SYSDATE, NEXT_DAY(SYSDATE, 'FRIDAY') FROM DUAL; -- 오류 : 현재언어가 KOREA이기 때문
...
SELECT EMP_NAME, HIRE_DATE, LAST_DAY(HIRE_DATE)
 ...
 ------------------------------------------------------
 /*
    *EXTPACT : 특정 날짜로부터 년도 |월|일 값을 추출하여 반환해주는 함수(반환형:NUMBER)
    
    EXTRACT(YEAR FROM DATE) : 년도 추출
    EXTRACT(MONTH FORM DATE): 월만 추출
    EXTRACT(DAY OF DATE) : 일만 추출
 */
 --EMPOLYEE에서 사원명, 입사년도, 입사월. 입사일 조회
 SELECT EMP_NAME,
...
/*
    [포맷]
    * 접두어 : L -> LOCAL(설정된 나라)의 화폐 단위
    *9 : 해당 자리의 숫자를 의미한다
        -해당 자리에 값이 없을 경우 소수점 이상은 공백, 소수점 이하는 0으로 표시
    *0 : 해당 자리의 숫자를 의미한다
        -해당 자리에 값이 없을 경우 0으로 표시하고, 숫자의 길이를 고저적으로 표시할 때 주로사용
    *FM : 해당 자리에 값이 없을 경우 자리차지를 하지 않음
*/
SELECT TO_CHAR(1234)+35, 1234+35 FROM DUAL; --문자는 왼쪽정렬, 숫자는 오른쪽 정렬
...
SELECT TO_CHAR(SYSDATE, 'PM') "AMERICAN" 
 ...
 --SELECT TO_CHAR(SYSDAYE, "YYYY"년 "MM"월 "DD"일 DAY) FROM DUAL;
...
SELECT TO_CHAR(SYSDATE,'DAY'), 
 ...
 /*
     * TO_DATE : 숫자나 문자를 날짜타입으로 변환
      TO_DATE(숫자|날짜, [포맷])
 */
  -- 환경설정 바꾸고 도구 -> 환경설정 -> 데이터베이스 -> NLS -> 날짜 포맷율 RRRR/MM/DD
...
SELECT EMP_NAME, NVL(DEPT_CODE, '부서X')
  FROM EMPLOYEE;
...
SELECT EXTRACT(YEAR FROM HIRE_DATE)
 ...
 -- 2001년,２００２년, 2003년, 2004년 개수
SELECT HIRE_DATE, DECODE(EXTRACT(YEAR FROM HIRE_DATE), '2001', 1, 0) "2001년",
                  DECODE(EXTRACT(YEAR FROM HIRE_DATE), '2002', 1, 0) "2002년",
                  DECODE(EXTRACT(YEAR FROM HIRE_DATE), '2003', 1, 0) "2003년",
                  DECODE(EXTRACT(YEAR FROM HIRE_DATE), '2004', 1, 0) "2004년"
 FROM EMPLOYEE;
