...
*/
-- 전 사원의 사번, 사원명, 성별(남/여),근무년수를 조회할 수 있는 뷰(VM_EMP_JOB) 생성
...
/*
    *단, DML(명령어로 조작이 불가능한 경우가 더 많음
    ...
    2) 뷰의 정의되어 있는 컬럼 중에 원본 테이블 상에 NOT NULL제약조건이 지정되어 있는 경우
    3) 산슬식 함수식으로 정의되어 있는 경우
    4) 그룹함수나 GROUP BY절이 포함되어 있는 경우
    5) DICTINCT 구문이 포함된 경우
    6) JOIN 이용하여 여러 테이블을 연결시켜놓은 경우
*/
...
--2) 뷰의 정의되어 있지 않은 컬럼 중에 원본테이블 상에 NOT NULL제약조건이 지정되어 있는 경우
CREATE OR REPLACE VIEW VM_JOB
AS SELECT JOB_NAME
    FROM JOB;
