/*
    ...
    * 트랜잭션
    ...
    DNL문 한개를 수행할 때 트랜잭션이 존재하면 해단 트랜잭션이 같이 묶어서 처리
    ...
    -COMMIT:한 트랜잭션에 담겨있는 변경사항들을 실제 DB에 반영시키겠다는 의미(의후에 트랜잭션은 사라짐)
    -RPLLBACK:한 트랜잭션 담겨있는 변경사항들을 삭제(취소)한 후 마지막 COMMIT시점으로 돌아감
    -SAVEPOINT:현재 이 시점에 해당 포인명으로 임시저장점을 정의해 두는 것
                ROLLBACK진생시 전체 변경사항을 삭제하는것이 아니라 일부만 삭제
*/
SELECT*FROM EMP_01;
