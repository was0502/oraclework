/*
	*DDL(DATA DEFINITION LANGUAGE) : 데이터 정의 언어
	오라클에서 제공하는 객체(OBJECT)를 만들고(CREATE),구조 자체를 삭제(DROP)하는 언어
	즉, 실제 데이터 값이 아닌 구조 자체를 정의하는 언어
	주로 DB관리자, 설계자가 사용함

	- 오라클에서 객체(구조) : 테이블(TABLE), 뷰(VIEW), 시퀀스(SEQUENCE), 인덱스(INDEX), 패키지(PACKAGE)
				트리거(TRIGGER),프로시저(PROCEDURE),함수(FUNTION),동의어(SYNONYM), 사용자(USER)
*/
--===============================================================
/*
	*CREATE
...
--사용자가 가지고 있는 테이블정보
--데이터 딕셔너리 : 다양한 객체들의 정보를 저장하고 있는 시스템 테이블둥
--[참고] USER_TABLE:이 사용자가 가지고 있는 테이블의 전반적인 구조를 확인할 수 있는 시스템 테이블
...
/*
    ...
    1. 개체 무결성 제약 조건 : NOT NULL, UNIQUE,
...
INSERT INTO MEM_NOTNULL VALUES(2,'user01',null,'김고잉','여',null,'user01@gmail.com'); --비밀번호의 null값 허용 안함
-- NOT NULL 제약조건에 위배되는 오류

INSERT INTO MEM_NOTNULL VALUES(1,'user03','pass03','김앤북','여','010-1234-0000','user03@gmail.com');
...
INSERT INTO MEM_UNIQUE4 VALUES (3, 'user03', 'pass03', '우재남', '남', '010-1234-5678', 'user01@gmail.com');
---> 성별이 유효한 값이 아니어도 들어감
...
INSERT INTO MEM VALUES(2,'user02','pass02','우채남','남',30);
...
/*
    *외래키(foreign key)제약조건
    ...
    --> 다른 테이블을 참조한다고 표현
    --> 주로 외래키 제약조건에 의해 테이블 가느이 관계가 형성됨
    
    >> 컬럼 레벨 방식
        컬럼명 자료형 [constraint 제약조건명]refernces 참조할테이블명[(참조할컬럼명)]
        
    >> 테이블 레벨 방식
        [constraint 제약조건명]foreign key(컬럼명)refernces 참조할테이블명[(참조할컬럼명)]
*/
...
INSERT INTO MEM2 VALUES(2,'user02','pass02','우채남','남',30);
...
INSERT INTO MEMBER2(MEM_ID,MEM_NO,MEM_AGE)VALUES(4,'user04',25);
...
--  FRIMARY KEY, DEFAULT, COMMENT는 복사안됨
...
----------------------------------------------------- 연습문제 ---------------------------------------------------
--  DDL 계정에서도서관리 프로그램을 만들기 위한 테이블들 만들기
--        이때, 제약조건에 이름을 부여할 것.
 --        각 컬럼에 주석달기
...
CREATE TABLE TB_PUBLISHER(
    PUB_NO NUMBER CONSTRAINT PABLISHER_PK PRIMARY KEY,
